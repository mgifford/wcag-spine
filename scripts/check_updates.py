#!/usr/bin/env python3
"""
WCAG Mirror Spine – Update Checker
...
def _request(url: str, method: str = "GET", timeout: int = 30):
    ...
    try:
        req = urllib.request.Request(
            ...
            headers={
                "User-Agent": (
                    "wcag-in-mermaid-update-checker/1.0 "
                    "(https://github.com/mgifford/wcag-in-mermaid)"
                )
            },
        )
...
def build_report(results: list[dict], run_date: str) -> str:
    ...
    lines: list[str] = [
        "## WCAG Mirror Spine – Monthly Update Check",
        "",
        f"**Run date:** {run_date}",
        "",
    ]
    ...
    lines += [
        ...
        "[WCAG Mirror Spine](https://github.com/mgifford/wcag-in-mermaid)_",
    ]
    return "\n".join(lines) + "\n"
